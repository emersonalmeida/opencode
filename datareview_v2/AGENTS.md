...
- Type-check global: `pnpm -r typecheck` (o `pnpm` via corepack pode pedir confirmação para baixar — usar `COREPACK_ENABLE_DOWNLOAD_PROMPT=0 pnpm ...`).
- Testes do domínio: `COREPACK_ENABLE_DOWNLOAD_PROMPT=0 pnpm --filter @v2/domain test` — roda `node --import tsx --test test/domain.test.ts`.
...
- Monorepo pnpm: `packages/contracts` (tipos `@v2/contracts`, sem deps); `packages/domain` (núcleo hexagonal puro); `apps/` ainda não criado.
...
- ADR-0002: `serpapi` e fallback multi-fonte (implementar SO as fallback das fontes que ja temos,
  nao o catalogo inteiro). Fonte primaria continua canonica; segredo `SERPAPI_KEY` byok no backend..
