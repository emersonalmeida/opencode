...
Núcleo de fontes unificado da v5 — uma evolução da v4 com três objetivos:
...
```
packages/
  contracts/  tipos compartilhados (única fonte de verdade)              @v5/contracts
  domain/     núcleo puro hexagonal: ports + pipeline (zero deps)        @v5/domain
  sources/    núcleo de fontes: catálogo + ativação + adaptadores           @v5/sources
apps/
...
COREPACK_ENABLE_DOWNLOAD_PROMPT=0 pnpm --filter @v5/sources gen:catalog
```
